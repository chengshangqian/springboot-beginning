...
public class AppFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
	...
	public Collection<ConfigAttribute> getAttributes(Object filterInvocation) throws IllegalArgumentException {
		logger.debug("调用了AppFilterInvocationSecurityMetadataSource  ===================>  getAttributes::filterInvocation");
		
		//从数据库或redis缓存中获取资源及其授权角色数
		...
		for(Menu menu : allMenus) {
			if(antPathMatcher.match(menu.getUrl(), requestUrl)) {
				...
				logger.debug("当前请求的URL在系统中定义的资源ID/URL为 => " + menu.getUrl());
				logger.debug("可以访问该资源的角色一共有 => " + rolesInfo);
		 ...
		 */
		logger.debug("当前请求URL未配置访问限制，返回默认角色：" + DEFAULT_CONFIG_ATTRIBUTE);
	...
	public Collection<ConfigAttribute> getAllConfigAttributes() {
		logger.debug("调用了AppFilterInvocationSecurityMetadataSource  ===================>  getAllConfigAttributes");
		return null;
	}
	...
	public boolean supports(Class<?> clazz) {
		logger.debug("调用了AppFilterInvocationSecurityMetadataSource  ===================>  supports::clazz");
