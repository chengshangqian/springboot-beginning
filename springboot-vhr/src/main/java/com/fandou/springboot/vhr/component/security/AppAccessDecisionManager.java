...
public class AppAccessDecisionManager implements AccessDecisionManager {
	...
	public void decide(Authentication authenticationUserWrapper, Object filterInvocation,
		 ...
		 * 见AppFilterInvocationSecurityMetadataSource类的配置，
		 * 所以这里至少会有一个角色AppFilterInvocationSecurityMetadataSource.
		 * DEFAULT_CONFIG_ATTRIBUTE）， 将资源所需的角色与用户的所有角色进行比较，如匹配则正常返回，否则抛出异常
		 */
		for (ConfigAttribute requiredRoles : requiredRolesWrapper) {
			String requiredRole = requiredRoles.getAttribute();
			logger.debug("当前请求URL[" + requestUrl + "] => 所需角色权限[" + requiredRole + "]");

			/*
			 * 响应未配置权限角色的资源（即缺省的权限角色的资源，见AppFilterInvocationSecurityMetadataSource类的配置）：
			 * 如果当前请求url是未配置权限角色的资源，假设业务系统对未配置权限的资源需要登录才能访问，
			 * 这里对authenticationUserWrapper对象进行检测
			 * 如果authenticationUserWrapper是UsernamePasswordAuthenticationToken实例，说明已经登录，
			 * 则正常返回即可 缺省的角色权限的资源如何响应，系统可以根据各自业务需要进行规划，比如直接开放访问，则这里不需要检查登录状态，直接正常返回即可
			 */
			...
			//}			
			if (AppFilterInvocationSecurityMetadataSource.DEFAULT_CONFIG_ATTRIBUTE.equals(requiredRole)) {
				if (authenticationUserWrapper instanceof AnonymousAuthenticationToken) {
			...
			for (GrantedAuthority userRoleWrapper : userRolesWrapper) {
				...
				logger.debug("当前用户角色权限[" + userRole + "] 是否匹配  URL权限[" + requiredRole + "] => " + (matched));
				if (matched) {
					logger.debug("当前用户角色与URL权限角色匹配成功！ ");
					return;
				}
			}
		}
